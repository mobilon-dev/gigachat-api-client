...
const message1 = `Привет! Набираю команду для удаленной занятости, доступно только для совершеннолетних. Адаптируемый график и солидный доход. Напишите + в лс, если хотите обсудить.`;
...
const message4 = `Всем привет 
Скажите пожалуйста , у кого я могу узнать почему реджектит новый шаблон ? 
В причине пишет "Invalid Format", хотелось бы конкретно понимать , что мешает`;
...
const messages = [
  ...
  {
   role: "user",
   content: message4.substring(0, 50),
  }
]
